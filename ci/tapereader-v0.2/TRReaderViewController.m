...
- (void)viewDidLoad {
    ...
    _scroll.maximumZoomScale = 4; _scroll.bouncesZoom = YES; [self.view addSubview:_scroll];
    ...
    if (self.reviewing) {
        ...
        _hideAllItem = [self button:@"모두 가리기" action:@selector(hideAll)];
...
- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated]; [_popover dismissPopoverAnimated:NO]; [_sheet dismissWithClickedButtonIndex:_sheet.cancelButtonIndex animated:NO]; [self persist];
}
...
- (void)fitPage {
    ...
    _scroll.minimumZoomScale = MIN(fit,1); _scroll.maximumZoomScale = MAX(4,fit*4);
...
- (void)displayPage:(NSUInteger)page {
    ...
    [self rebuildTapes]; [self modeChanged]; [self fitPage]; [self updatePageLabel]; [self persist];
}
- (void)updatePageLabel {
    ...
    else [self updateSelectionChrome];
}
...
- (void)updateModeChrome {
    if (self.reviewing || !_modes) return;
    _thicknessItem.enabled = [self isEditingMode];
    [self updateSelectionChrome];
}
...
- (CGPoint)canvasPointForGesture:(UIGestureRecognizer *)g {
    CGPoint inReader = [g locationInView:self.view];
    CGPoint p = [_canvas convertPoint:inReader fromView:self.view];
...
- (void)createTape:(UIPanGestureRecognizer *)g {
    ...
    if (g.state == UIGestureRecognizerStateBegan) {
        CGPoint current = [g locationInView:self.view], delta = [g translationInView:self.view];
        CGPoint down = CGPointMake(current.x-delta.x,current.y-delta.y);
        _start = [_canvas convertPoint:down fromView:self.view];
        _start.x = TRClamp(_start.x,0,s.width); _start.y = TRClamp(_start.y,0,s.height);
    ...
    if (g.state == UIGestureRecognizerStateEnded) {
        if (width >= 8) {
            ...
            _draft.selectedForEditing = YES; [self persist]; [self updateSelectionChrome];
...
- (void)tapeViewTapped:(TRTapeView *)view {
    if (self.reviewing) {
        ...
        view.tape.revealed = !view.tape.revealed; [view refresh]; [self updateReviewAnswerTitle]; [self persist]; return;
    }
    ...
    view.tape.revealed = !view.tape.revealed; [view refresh]; [self persist];
}
- (void)tapeViewChanged:(TRTapeView *)view {
    ...
    [self updateSelectionChrome]; [self persist];
}
...
- (void)showSelectionActions {
    ...
    _sheet = [[UIActionSheet alloc] initWithTitle:[NSString stringWithFormat:@"선택한 %lu개 테이프",(unsigned long)selected.count] delegate:self cancelButtonTitle:nil destructiveButtonTitle:nil otherButtonTitles:@"★ 모름",@"? 애매함",@"✓ 외움",@"모두 가리기",@"두께 맞추기",@"삭제",nil];
    _sheet.tag = 2; _sheet.destructiveButtonIndex = 5; _sheet.cancelButtonIndex = [_sheet addButtonWithTitle:@"취소"];
...
- (void)actionSheet:(UIActionSheet *)sheet didDismissWithButtonIndex:(NSInteger)index {
    ...
    if (sheet.tag == 2) {
        ...
        else if (index == 5) { for (TRTape *t in selected) [self.document.tapes removeObject:t]; [_selectedTapeIDs removeAllObjects]; }
        [self rebuildTapes]; [self persist]; return;
    }
    if (!_selected) return;
    if (index <= 2) { _selected.tape.memoryState = index+1; [_selected refresh]; [self persist]; }
    else if (index == 3) [self showThicknessForTape:_selected];
    else if (index == 4) { [self.document.tapes removeObject:_selected.tape]; [_selectedTapeIDs removeObject:_selected.tape.identifier]; [_selected removeFromSuperview]; _selected = nil; [self updateSelectionChrome]; [self persist]; }
}
...
- (void)thicknessEnded:(UISlider *)slider { [self persist]; }
- (void)popoverControllerDidDismissPopover:(UIPopoverController *)popover { [self persist]; _selected = nil; _batchThickness = NO; }
- (void)hideAll { for (TRTape *t in self.document.tapes) t.revealed = NO; [self rebuildTapes]; [self persist]; }
...
- (void)toggleReviewAnswer {
    ...
    [[self visibleViewForTape:t] refresh]; [self updateReviewAnswerTitle]; [self persist];
}
- (void)showReviewIndex:(NSUInteger)index {
    ...
    if (_page != t.pageIndex) [self displayPage:t.pageIndex]; else { [self rebuildTapes]; [self focusReviewTape]; [self updatePageLabel]; [self persist]; }
}
...
- (void)rate:(TRMemoryState)state {
    ...
    t.memoryState = state; t.revealed = NO; [self persist];
    if (_reviewIndex+1 < _reviewTapes.count) [self reviewNext];
    else { [self rebuildTapes]; [self updateReviewAnswerTitle]; [self message:@"복습을 마쳤습니다. 뒤로 돌아가 새 복습을 시작할 수 있습니다."]; }
}
...
- (void)didReceiveMemoryWarning {
    [super didReceiveMemoryWarning]; [self persist];
...
- (void)dealloc { [[NSNotificationCenter defaultCenter] removeObserver:self]; _scroll.delegate = nil; _popover.delegate = nil; _sheet.delegate = nil; }
@end
