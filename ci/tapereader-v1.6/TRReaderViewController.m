...
@implementation TRReaderViewController {
    ...
    UIBarButtonItem *_studyPreviousItem;
    UIBarButtonItem *_studyNextItem;
    UIBarButtonItem *_studyAnswerItem;
    UIBarButtonItem *_studyUnknownItem;
    UIBarButtonItem *_studyUnsureItem;
    UIBarButtonItem *_studyKnownItem;
    UIBarButtonItem *_shuffleItem;
    NSArray *_studyTapes;
    NSUInteger _studyIndex;
    NSInteger _lastModeIndex;
    ...
    BOOL _choosingReview;
    NSMutableArray *_reviewPickIDs;
    UIBarButtonItem *_reviewPickStartItem;
    UIBarButtonItem *_reviewPickCancelItem;
    UIBarButtonItem *_deleteSelectionItem;
...
- (id)initWithDocument:(TRDocument *)document reviewTapes:(NSArray *)tapes {
    if ((self = [super init])) {
        ...
        _reviewPickIDs = [NSMutableArray array];
        _lastModeIndex = -1;
...
- (BOOL)isEditingMode { return !self.reviewing && !_choosingReview && _modes && _modes.selectedSegmentIndex == 1; }
- (BOOL)isStudyMode { return !self.reviewing && !_choosingReview && _modes && _modes.selectedSegmentIndex == 0; }
- (void)viewDidLoad {
    ...
    _scroll.maximumZoomScale = 6; _scroll.bouncesZoom = YES; _scroll.decelerationRate = UIScrollViewDecelerationRateFast;
    _scroll.delaysContentTouches = NO; _scroll.canCancelContentTouches = YES;
    ...
    if (self.reviewing) {
        ...
        _modes.segmentedControlStyle = UISegmentedControlStyleBar;
        _modes.selectedSegmentIndex = self.document.tapes.count ? 0 : 1;
        [_modes addTarget:self action:@selector(modeChanged) forControlEvents:UIControlEventValueChanged];
        _modes.frame = CGRectMake(0,0,180,30);
        self.navigationItem.titleView = _modes;
        _reviewItem = [self button:@"복습" action:@selector(openReview)];
        _reviewPickStartItem = [self button:@"선택 0개 · 시작" action:@selector(startPickedReview)];
        _reviewPickCancelItem = [self button:@"취소" action:@selector(cancelReviewPicker)];
        _deleteSelectionItem = [self button:@"삭제" action:@selector(deleteSelectedTapes)];
        _shareItem = [self button:@"내보내기" action:@selector(showExportActions)];
        ...
        _studyPreviousItem = [self button:@"이전" action:@selector(studyPrevious)];
        _studyNextItem = [self button:@"다음" action:@selector(studyNext)];
        _studyAnswerItem = [self button:@"정답 보기" action:@selector(toggleStudyAnswer)];
        _studyUnknownItem = [self button:@"★ 모름" action:@selector(studyRateUnknown)];
        _studyUnsureItem = [self button:@"? 애매" action:@selector(studyRateUnsure)];
        _studyKnownItem = [self button:@"✓ 외움" action:@selector(studyRateKnown)];
        _shuffleItem = [self button:@"섞기" action:@selector(shuffleStudy)];
        ...
        _toolbar.items = @[_previous,[self space],_pageItem,[self space],_thicknessItem,[self space],_next];
        if ([self isStudyMode]) [self prepareStudySessionPreservingPage:YES];
        _lastModeIndex = _modes.selectedSegmentIndex;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!self.reviewing && _modes && !_choosingReview) self.navigationItem.titleView = _modes;
    if (_canvas) [self rebuildTapes]; else if ([self isViewLoaded] && _pageCount) [self displayPage:_page];
}
...
- (void)fitPage {
    ...
    else if ([self isStudyMode]) [self focusStudyTape];
}
- (void)centerPage {
...
- (void)updatePageLabel {
    ...
    else if ([self isEditingMode] && self.document.tapes.count == 0)
        _pageItem.title = @"답 위를 가로로 드래그";
    else
        _pageItem.title = [NSString stringWithFormat:@"%lu / %lu",(unsigned long)_page+1,(unsigned long)_pageCount];
    ...
    else { [self updateSelectionChrome]; [self updatePageRevealTitle]; if ([self isStudyMode]) [self updateStudyControls]; }
}
...
- (void)updateSelectionChrome {
    ...
    if (_choosingReview) { [self updateReviewPickerChrome]; return; }
    BOOL editing = [self isEditingMode];
    _thicknessItem.enabled = editing;
    if (!editing) {
        self.navigationItem.rightBarButtonItems = @[_shareItem,_reviewItem,_shuffleItem];
        return;
    }
    ...
    self.navigationItem.rightBarButtonItems = selected.count ? @[_deleteSelectionItem,_selectionActionsItem,_selectAllItem] : @[_shareItem,_selectAllItem];
...
- (void)rebuildTapes {
    ...
    for (TRTape *t in self.document.tapes) if (t.pageIndex == _page) {
        ...
        v.peelEnabled = [self isStudyMode];
        v.selectedForEditing = _choosingReview ? [_reviewPickIDs containsObject:t.identifier] : [_selectedTapeIDs containsObject:t.identifier];
        NSUInteger reviewPosition = _choosingReview ? [_reviewPickIDs indexOfObject:t.identifier] : NSNotFound;
        v.reviewOrder = reviewPosition == NSNotFound ? 0 : (NSInteger)reviewPosition + 1;
        v.focused = !_choosingReview && ((self.reviewing && _reviewTapes.count && t == _reviewTapes[_reviewIndex]) ||
                    ([self isStudyMode] && _studyTapes.count && _studyIndex < _studyTapes.count && t == _studyTapes[_studyIndex]));
    ...
    if ([self isStudyMode]) [self updateStudyControls];
}
- (void)modeChanged {
    if (_choosingReview) {
        _create.enabled = NO;
        _scroll.panGestureRecognizer.minimumNumberOfTouches = 1;
        for (TRTapeView *v in _canvas.subviews) if ([v isKindOfClass:[TRTapeView class]]) {
            v.editable = NO;
            v.peelEnabled = NO;
            v.focused = NO;
            v.selectedForEditing = [_reviewPickIDs containsObject:v.tape.identifier];
            NSUInteger p = [_reviewPickIDs indexOfObject:v.tape.identifier];
            v.reviewOrder = p == NSNotFound ? 0 : (NSInteger)p + 1;
            [v refresh];
        }
        [self updateReviewPickerChrome];
        return;
    }
    BOOL editing = [self isEditingMode];
    NSInteger currentMode = _modes ? _modes.selectedSegmentIndex : -1;
    BOOL actuallyChanged = currentMode != _lastModeIndex;
    _lastModeIndex = currentMode;
    if (!editing) {
        [_selectedTapeIDs removeAllObjects];
        if (actuallyChanged) [self prepareStudySessionPreservingPage:YES];
    }
    _create.enabled = editing;
    _scroll.panGestureRecognizer.minimumNumberOfTouches = editing ? 2 : 1;
    for (TRTapeView *v in _canvas.subviews) if ([v isKindOfClass:[TRTapeView class]]) {
        v.editable = editing;
        v.peelEnabled = !editing && [self isStudyMode];
        v.selectedForEditing = editing && [_selectedTapeIDs containsObject:v.tape.identifier];
        v.focused = [self isStudyMode] && _studyTapes.count && _studyIndex < _studyTapes.count && v.tape == _studyTapes[_studyIndex];
        [v refresh];
    }
    [self updateModeChrome]; [self updateSelectionChrome]; [self updatePageLabel];
    if ([self isStudyMode]) [self focusStudyTape];
}
- (void)updateModeChrome {
    ...
    self.navigationItem.titleView = _modes;
    if (editing) {
        _toolbar.items = @[_previous,[self space],_pageItem,[self space],_thicknessItem,[self space],_fitItem,[self space],_next];
        self.title = self.document.title;
    } else {
        _toolbar.items = @[_studyPreviousItem,[self space],_studyAnswerItem,[self space],_studyUnknownItem,_studyUnsureItem,_studyKnownItem,[self space],_studyNextItem];
        [self updateStudyControls];
    }
    [self updateSelectionChrome]; [self updatePageRevealTitle];
}
...
- (void)createTape:(UIPanGestureRecognizer *)g {
    ...
    if (g.state == UIGestureRecognizerStateEnded) {
        CGFloat zoom = MAX(.25,_scroll.zoomScale);
        CGFloat minimumScreenWidth = 14.0;
        CGFloat minimumCanvasWidth = minimumScreenWidth / zoom;
        if (width >= minimumCanvasWidth) {
            [self.document.tapes addObject:_draft.tape];
            [_selectedTapeIDs removeAllObjects]; [_selectedTapeIDs addObject:_draft.tape.identifier];
            _draft.selectedForEditing = YES;
            [self schedulePersist]; [self updateSelectionChrome]; [self updatePageLabel];
        } else {
            [_draft removeFromSuperview];
        }
        _draft = nil;
    }
}
...
- (void)alertView:(UIAlertView *)alert clickedButtonAtIndex:(NSInteger)button {
    ...
    if (alert.tag == 20 && button == 1) {
        NSArray *selected = [[self selectedTapesOnCurrentPage] copy];
        for (TRTape *t in selected) [self.document.tapes removeObject:t];
        [_selectedTapeIDs removeAllObjects];
        [self rebuildTapes]; [self updatePageLabel]; [self schedulePersist];
    }
}
- (void)tapeViewTapped:(TRTapeView *)view {
    if (_choosingReview) {
        if ([_reviewPickIDs containsObject:view.tape.identifier]) [_reviewPickIDs removeObject:view.tape.identifier];
        else [_reviewPickIDs addObject:view.tape.identifier];
        [self rebuildTapes];
        [self updateReviewPickerChrome];
        return;
    }
    ...
    if ([self isStudyMode]) {
        NSUInteger tappedIndex = [_studyTapes indexOfObjectIdenticalTo:view.tape];
        if (tappedIndex != NSNotFound && tappedIndex != _studyIndex) {
            TRTape *old = [self currentStudyTape]; old.revealed = NO;
            _studyIndex = tappedIndex; [self rebuildTapes]; [self focusStudyTape];
            view = [self visibleViewForTape:view.tape];
        }
        TRTape *t = [self currentStudyTape];
        if (t) { t.revealed = !t.revealed; [[self visibleViewForTape:t] refresh]; [self updateStudyControls]; [self schedulePersist]; }
        return;
    }
...
- (void)tapeViewHeld:(TRTapeView *)view {
    if (_choosingReview || [self isStudyMode]) return;
...
- (void)toggleSelectAll {
    ...
    if (_choosingReview) {
        ...
        BOOL allPicked = YES;
        for (TRTape *t in pageTapes) if (![_reviewPickIDs containsObject:t.identifier]) { allPicked = NO; break; }
        if (allPicked) for (TRTape *t in pageTapes) [_reviewPickIDs removeObject:t.identifier];
        else for (TRTape *t in pageTapes) [_reviewPickIDs addObject:t.identifier];
        [self rebuildTapes]; [self updateReviewPickerChrome]; return;
    }
    if (![self isEditingMode]) return;
    if (!pageTapes.count) { [self message:@"이 페이지에는 테이프가 없습니다."]; return; }
...
- (void)actionSheet:(UIActionSheet *)sheet didDismissWithButtonIndex:(NSInteger)index {
    ...
    if (sheet.tag == 3) {
        NSError *error = nil; NSURL *url = nil;
        if (index == 0) url = [[TRDocumentStore sharedStore] exportPDFWithTapesForDocument:self.document error:&error];
        else if (index == 1) url = [[TRDocumentStore sharedStore] backupURLForDocument:self.document error:&error];
        if (url) [self presentExportURL:url];
        else if (error) [self message:[NSString stringWithFormat:@"내보내기 실패: %@",error.localizedDescription]];
        return;
    }
    if (sheet.tag == 2) {
        ...
        [self rebuildTapes]; [self updatePageLabel]; [self schedulePersist]; return;
    }
    if (!_selected) return;
    ...
    else if (index == 4) { [self.document.tapes removeObject:_selected.tape]; [_selectedTapeIDs removeObject:_selected.tape.identifier]; [_selected removeFromSuperview]; _selected = nil; [self updateSelectionChrome]; [self updatePageLabel]; [self schedulePersist]; }
}
...
- (void)prepareStudySessionPreservingPage:(BOOL)preservePage {
    TRTape *oldTarget = [self currentStudyTape];
    NSMutableArray *items = [NSMutableArray arrayWithArray:self.document.tapes];
    [items sortUsingComparator:^NSComparisonResult(TRTape *a, TRTape *b) {
        if (a.pageIndex != b.pageIndex) return a.pageIndex < b.pageIndex ? NSOrderedAscending : NSOrderedDescending;
        CGFloat ay = a.normalizedRect.origin.y + a.normalizedRect.size.height*.5;
        CGFloat by = b.normalizedRect.origin.y + b.normalizedRect.size.height*.5;
        if (ay != by) return ay < by ? NSOrderedAscending : NSOrderedDescending;
        CGFloat ax = a.normalizedRect.origin.x + a.normalizedRect.size.width*.5;
        CGFloat bx = b.normalizedRect.origin.x + b.normalizedRect.size.width*.5;
        if (ax != bx) return ax < bx ? NSOrderedAscending : NSOrderedDescending;
        return [a.identifier compare:b.identifier];
    }];
    _studyTapes = [items copy];
    for (TRTape *t in _studyTapes) t.revealed = NO;
    if (!_studyTapes.count) { _studyIndex = 0; [self updateStudyControls]; return; }
    NSUInteger newIndex = NSNotFound;
    if (oldTarget) newIndex = [_studyTapes indexOfObjectIdenticalTo:oldTarget];
    if (newIndex == NSNotFound && preservePage) {
        for (NSUInteger i=0; i<_studyTapes.count; i++) {
            TRTape *t = _studyTapes[i];
            if (t.pageIndex >= _page) { newIndex = i; break; }
        }
    }
    if (newIndex == NSNotFound) newIndex = 0;
    _studyIndex = newIndex;
    [self updateStudyControls];
}
- (TRTape *)currentStudyTape {
    if (!_studyTapes.count || _studyIndex >= _studyTapes.count) return nil;
    return _studyTapes[_studyIndex];
}
- (void)updateStudyControls {
    if (![self isStudyMode]) return;
    TRTape *t = [self currentStudyTape];
    BOOL has = t != nil;
    _studyPreviousItem.enabled = has && _studyIndex > 0;
    _studyNextItem.enabled = has && _studyIndex+1 < _studyTapes.count;
    _studyAnswerItem.enabled = has;
    _shuffleItem.enabled = _studyTapes.count > 1;
    BOOL revealed = has && t.revealed;
    _studyAnswerItem.title = revealed ? @"다시 가리기" : @"정답 보기";
    _studyUnknownItem.enabled = revealed; _studyUnsureItem.enabled = revealed; _studyKnownItem.enabled = revealed;
    if (!has) self.title = @"학습 · 테이프 없음";
    else self.title = [NSString stringWithFormat:@"학습 %lu / %lu · %lu쪽",(unsigned long)_studyIndex+1,(unsigned long)_studyTapes.count,(unsigned long)t.pageIndex+1];
    self.navigationItem.titleView = _modes;
}
- (void)focusStudyTape {
    if (![self isStudyMode] || !_canvas) return;
    TRTape *t = [self currentStudyTape]; if (!t || t.pageIndex != _page) return;
    CGRect n = t.normalizedRect; CGSize size = _canvas.bounds.size;
    CGRect focus = CGRectMake(n.origin.x*size.width,n.origin.y*size.height,n.size.width*size.width,n.size.height*size.height);
    focus = CGRectInset(focus,-95,-145); focus = CGRectIntersection(focus,_canvas.bounds);
    if (focus.size.width > 8 && focus.size.height > 8) [_scroll zoomToRect:focus animated:YES];
}
- (void)showStudyIndex:(NSUInteger)index {
    if (index >= _studyTapes.count) return;
    TRTape *old = [self currentStudyTape]; if (old) old.revealed = NO;
    _studyIndex = index; TRTape *t = [self currentStudyTape]; t.revealed = NO;
    if (_page != t.pageIndex) [self displayPage:t.pageIndex];
    else { [self rebuildTapes]; [self focusStudyTape]; [self updateStudyControls]; }
    [self schedulePersist];
}
- (void)studyPrevious { if (_studyIndex > 0) [self showStudyIndex:_studyIndex-1]; }
- (void)studyNext { if (_studyIndex+1 < _studyTapes.count) [self showStudyIndex:_studyIndex+1]; }
- (void)toggleStudyAnswer {
    TRTape *t = [self currentStudyTape]; if (!t) { [self message:@"편집 모드에서 먼저 테이프를 만들어 주세요."]; return; }
    TRTapeView *view = [self visibleViewForTape:t];
    if (view) { [view togglePeelAnimated]; return; }
    t.revealed = !t.revealed; [self updateStudyControls]; [self schedulePersist];
}
- (void)studyRate:(TRMemoryState)state {
    TRTape *t = [self currentStudyTape]; if (!t) return;
    if (!t.revealed) { [self message:@"정답을 확인한 뒤 상태를 선택하세요."]; return; }
    t.memoryState = state; t.revealed = NO; [self schedulePersist];
    if (_studyIndex+1 < _studyTapes.count) { [self showStudyIndex:_studyIndex+1]; return; }
    NSUInteger unknown=0, unsure=0, known=0, unmarked=0;
    for (TRTape *item in _studyTapes) {
        if (item.memoryState == TRUnknown) unknown++;
        else if (item.memoryState == TRUnsure) unsure++;
        else if (item.memoryState == TRKnown) known++;
        else unmarked++;
    }
    [self persist]; [self rebuildTapes]; [self updateStudyControls];
    [self message:[NSString stringWithFormat:@"학습 완료\n외움 %lu · 애매 %lu · 모름 %lu · 미분류 %lu\n\n‘복습’에서 어려운 항목만 다시 볼 수 있습니다.",(unsigned long)known,(unsigned long)unsure,(unsigned long)unknown,(unsigned long)unmarked]];
}
- (void)studyRateUnknown { [self studyRate:TRUnknown]; }
- (void)studyRateUnsure { [self studyRate:TRUnsure]; }
- (void)studyRateKnown { [self studyRate:TRKnown]; }
- (void)shuffleStudy {
    if (_studyTapes.count < 2) return;
    NSMutableArray *items = [_studyTapes mutableCopy];
    for (NSUInteger i=items.count-1; i>0; i--) {
        NSUInteger j = arc4random_uniform((u_int32_t)(i+1));
        [items exchangeObjectAtIndex:i withObjectAtIndex:j];
    }
    for (TRTape *t in items) t.revealed = NO;
    _studyTapes = [items copy]; _studyIndex = 0;
    TRTape *t = [self currentStudyTape];
    if (t.pageIndex != _page) [self displayPage:t.pageIndex];
    else { [self rebuildTapes]; [self focusStudyTape]; [self updateStudyControls]; }
}
- (void)deleteSelectedTapes {
    NSArray *selected = [self selectedTapesOnCurrentPage];
    if (!selected.count) { [self message:@"삭제할 테이프를 먼저 탭해 선택하세요."]; return; }
    NSString *message = selected.count == 1 ? @"선택한 테이프를 삭제할까요?" :
        [NSString stringWithFormat:@"선택한 %lu개 테이프를 삭제할까요?",(unsigned long)selected.count];
    UIAlertView *a = [[UIAlertView alloc] initWithTitle:@"테이프 삭제" message:message delegate:self cancelButtonTitle:@"취소" otherButtonTitles:@"삭제",nil];
    a.tag = 20; [a show];
}
- (void)showExportActions {
    [self persist];
    [_sheet dismissWithClickedButtonIndex:_sheet.cancelButtonIndex animated:NO];
    _sheet = [[UIActionSheet alloc] initWithTitle:@"내보내기 / 백업" delegate:self cancelButtonTitle:nil destructiveButtonTitle:nil
                               otherButtonTitles:@"테이프가 붙은 PDF 내보내기",@"전체 백업 만들기",nil];
    _sheet.tag = 3;
    _sheet.cancelButtonIndex = [_sheet addButtonWithTitle:@"취소"];
    [_sheet showFromBarButtonItem:_shareItem animated:YES];
}
- (void)presentExportURL:(NSURL *)url {
    if (!url) return;
    _documentInteraction = [UIDocumentInteractionController interactionControllerWithURL:url];
    BOOL shown = [_documentInteraction presentOptionsMenuFromBarButtonItem:_shareItem animated:YES];
    if (!shown) {
        [self message:[NSString stringWithFormat:@"파일을 만들었습니다.\n\n%@/Export/%@",[TRDocumentStore sharedStore].rootPath,url.lastPathComponent]];
    }
}
- (void)updateReviewPickerChrome {
    if (!_choosingReview) return;
    NSUInteger count = _reviewPickIDs.count;
    self.navigationItem.titleView = nil;
    self.title = [NSString stringWithFormat:@"복습 순서 · %lu개",(unsigned long)count];

    NSArray *pageTapes = [self tapesOnCurrentPage];
    BOOL allPage = pageTapes.count > 0;
    for (TRTape *t in pageTapes) if (![_reviewPickIDs containsObject:t.identifier]) { allPage = NO; break; }
    _selectAllItem.title = allPage ? @"이 페이지 해제" : @"이 페이지 전체";

    _reviewPickStartItem.enabled = count > 0;
    _reviewPickStartItem.title = count ? [NSString stringWithFormat:@"1→%lu 순서로 시작",(unsigned long)count] : @"원하는 순서대로 테이프를 탭";
    self.navigationItem.rightBarButtonItems = @[_reviewPickCancelItem,_selectAllItem];
    _toolbar.items = @[_previous,[self space],_pageItem,[self space],_reviewPickStartItem,[self space],_next];
    _previous.enabled = _page > 0;
    _next.enabled = _page+1 < _pageCount;
}
- (void)openReview {
    if (!self.document.tapes.count) { [self message:@"먼저 편집 모드에서 테이프를 만들어 주세요."]; return; }
    [_reviewPickIDs removeAllObjects];
    _choosingReview = YES;
    _modes.enabled = NO;
    [_selectedTapeIDs removeAllObjects];
    [self rebuildTapes];
...
- (void)startPickedReview {
    if (!_choosingReview || !_reviewPickIDs.count) { [self message:@"복습할 테이프를 원하는 순서대로 눌러 주세요."]; return; }
    NSMutableArray *items = [NSMutableArray arrayWithCapacity:_reviewPickIDs.count];
    for (NSString *identifier in _reviewPickIDs) {
        for (TRTape *t in self.document.tapes) {
            if ([t.identifier isEqual:identifier]) { [items addObject:t]; break; }
        }
    }
    if (!items.count) { [self message:@"선택한 테이프를 찾을 수 없습니다."]; return; }
    [self persist];
    TRReaderViewController *review = [[TRReaderViewController alloc] initWithDocument:self.document reviewTapes:items];
...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    [_documentInteraction dismissMenuAnimated:NO];
}
