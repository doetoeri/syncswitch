...
- (void)viewDidLoad {
    ...
    help.text = @"학습: 테이프 탭으로 정답 확인 · 편집: 드래그로 테이프 추가\n복습: 원하는 순서대로 테이프를 눌러 순서 지정\n내보내기에서 테이프 PDF·전체 백업 생성, + 로 PDF/백업 복원"; self.tableView.tableFooterView = help;
}
...
- (void)addPDF {
    ...
    if (!urls.count) { [self message:[NSString stringWithFormat:@"PDF 또는 .tapereaderbackup 파일을 ‘다음에서 열기’로 TapeReader에 보내세요.\n\n또는 아래 폴더에 파일을 복사한 뒤 +를 누르세요.\n%@/Import\n\n백업을 가져오면 PDF와 테이프 상태가 함께 복원됩니다.",[TRDocumentStore sharedStore].rootPath]]; return; }
    ...
    [self.tableView reloadData]; [self message:[NSString stringWithFormat:@"%lu개 파일을 가져왔습니다.%@",(unsigned long)count, failures.count ? [@"\n" stringByAppendingString:[failures componentsJoinedByString:@"\n"]] : @""]];
}
