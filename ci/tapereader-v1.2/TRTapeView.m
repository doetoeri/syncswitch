...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event {
    CGFloat extraX = MAX(12,(44-self.bounds.size.width)*.5);
    CGFloat extraY = MAX(14,(44-self.bounds.size.height)*.5);
...
- (void)dragged:(UIPanGestureRecognizer *)g {
    ...
    if (g.state == UIGestureRecognizerStateBegan) {
        ...
        if (p.x < handle) _resizeEdge = -1;
        else if (p.x > self.bounds.size.width-handle) _resizeEdge = 1;
        else _resizeEdge = 0;
    ...
    CGFloat minimum = MIN(16/s.width,1);
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.editable) {
        CGContextSetRGBFillColor(c,.20,.18,.12,.60);
        CGFloat h = MAX(6,b.size.height-8);
        CGFloat y = MAX(2,(b.size.height-h)*.5);
        CGContextFillRect(c,CGRectMake(2,y,4,h));
        CGContextFillRect(c,CGRectMake(MAX(0,b.size.width-6),y,4,h));
    }
