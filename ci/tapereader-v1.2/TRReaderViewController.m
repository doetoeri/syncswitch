...
- (void)viewDidLoad {
    ...
    if (self.reviewing) {
        ...
        [self prepareStudySessionPreservingPage:YES];
        _lastModeIndex = 0;
    }
...
- (void)updateModeChrome {
    ...
    if (editing) {
        _toolbar.items = @[_previous,[self space],modeItem,[self space],_pageItem,[self space],_thicknessItem,[self space],_next];
        self.title = self.document.title;
    } else {
        _toolbar.items = @[modeItem,[self space],_studyPreviousItem,[self space],_studyAnswerItem,[self space],_studyUnknownItem,_studyUnsureItem,_studyKnownItem,[self space],_studyNextItem];
