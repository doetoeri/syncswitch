...
- (id)initWithDocument:(TRDocument *)document reviewTapes:(NSArray *)tapes {
    if ((self = [super init])) {
        ...
        _reviewPickIDs = [NSMutableSet set];
        _lastModeIndex = -1;
...
- (void)updateSelectionChrome {
    ...
    if (!editing) {
        self.navigationItem.rightBarButtonItems = @[_reviewItem,_shuffleItem];
        return;
    }
...
- (void)tapeViewTapped:(TRTapeView *)view {
    if (_choosingReview) {
        ...
        view.selectedForEditing = [_reviewPickIDs containsObject:view.tape.identifier];
...
- (void)updateReviewPickerChrome {
    ...
    self.title = [NSString stringWithFormat:@"복습 선택 · %lu개",(unsigned long)count];
    ...
    _reviewPickStartItem.title = count ? [NSString stringWithFormat:@"선택 %lu개 복습",(unsigned long)count] : @"테이프를 탭해 선택";
...
- (void)startPickedReview {
    if (!_choosingReview || !_reviewPickIDs.count) { [self message:@"복습할 테이프를 먼저 선택하세요."]; return; }
    NSMutableArray *items = [NSMutableArray array];
    for (TRTape *t in self.document.tapes) if ([_reviewPickIDs containsObject:t.identifier]) [items addObject:t];
    [items sortUsingComparator:^NSComparisonResult(TRTape *a, TRTape *b) {
        if (a.pageIndex != b.pageIndex) return a.pageIndex < b.pageIndex ? NSOrderedAscending : NSOrderedDescending;
        CGFloat ay = a.normalizedRect.origin.y + a.normalizedRect.size.height*.5;
        CGFloat by = b.normalizedRect.origin.y + b.normalizedRect.size.height*.5;
